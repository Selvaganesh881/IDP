...
async def get_session_by_hash(
    db: aiosqlite.Connection, hash: str
...
async def delete_session_by_hash(
    ...
    await db.execute("DELETE * FROM pii_masking_sessions WHERE file_hash = ?", (hash,))
...
async def get_mappings_by_session_id(
    db: aiosqlite.Connection, session_id: str
) -> list[dict[str, str]]:

    async with db.execute(
        "SELECT * FROM pii_masking_mappings WHERE session_id = ? ORDER BY id",
        (session_id,),
    ) as c:
